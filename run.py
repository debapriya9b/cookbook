...
def view(id):
    ...
    return render_template('view.html', 
                           title='View Full Recipe', recipe=recipe)
...
def insert_recipe():
    ...
    data.update({'recipe_ingredients': request.form.
                 getlist('recipe_ingredients[]')})
    data.update({'recipe_procedure': request.form.
                 getlist('recipe_procedure[]')})
...
def update_recipe(id):
    ...
    recipe.update({'_id': ObjectId(id)},
                 ...
                 'recipe_ingredients': request.
                 form.getlist('recipe_ingredients[]'),
                 'recipe_procedure': request.
                 form.getlist('recipe_procedure[]'),
                 'recipe_category': request.form.get('recipe_category'),
                 'recipe_time': request.form.get('recipe_time'),
                 'recipe_difficulty_level': request.
                 form.get('recipe_difficulty_level'),
...
def register():
    ...
    A check is performed to verify that the user email 
    doesn't already exist.
    If the user already exists, the register page is 
    rendered with a user_exists error.
    If a user does not exist, the user is added to the 
    database and the login page is rendered.
    """
    # check for logged in user
    email = session.get('email')
    if email:
    ...
    if request.method == 'POST':
        ...
        if mongo.db.users.find_one({"email": email}):
            return render_template('register.html', 
                                   title='Register', error="user_exists")
        else:
            mongo.db.users.insert_one(user)
            return render_template('login.html', 
                                   title='Login', user=user, password=password)
